	...
	func adjust(delta: Decimal) {
		...
		if 0.5 < abs(delta) {
			...
			guard clock_gettime(CLOCK_REALTIME, &ts) == 0 else {
				os_log("adjust step get failed", log: facility, type: .info)
				return
			}
			ts.tv_sec += sec
			ts.tv_nsec += nsec
			guard clock_settime(CLOCK_REALTIME, &ts) == 0 else {
				os_log("adjust step set failed", log: facility, type: .info)
				return
			}
		} else {
			...
			guard adjtime(&tv, nil) == 0 else {
				os_log("adjust slew failed", log: facility, type: .info)
				return
			}
		}
	}
